...
#echo -e "CAPITA_LMS: setquarantine - set quarantine status \n last 20mins of returned items changed to quarantine status \n \n  $(cat \/usr/opt/blcmp/local/utils/bin/set_quarantine/logs/$new_fileName)" | mailx -s "CAPITA_LMS setquarantine log" -r "LIBSYS<librarysystems@yorksj.ac.uk>" librarysystems@yorksj.ac.uk