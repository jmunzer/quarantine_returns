...
#echo -e "CAPITA_LMS: resetloanable - reset loanable status \n stock quarantined for 72 hours, set to loanable status \n \n  $(cat \/usr/opt/blcmp/local/utils/bin/reset_loanable/logs/$new_fileName)" | mailx -s "CAPITA_LMS resetloanable log" -r "LIBSYS<librarysystems@yorksj.ac.uk>" librarysystems@yorksj.ac.uk
